...
import { getUpdates } from 'apiSdk/updates';
...
function DashboardPage() {
  ...
  const updateStat = useSWR<PaginatedInterface<UpdateInterface>>(
    () => `/updates`,
    () => getUpdates({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Updates',
      result: updateStat?.data?.totalCount,
      isLoading: updateStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
